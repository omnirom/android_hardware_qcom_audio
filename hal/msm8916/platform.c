...
void resolveConfigFile(char file_name[MIXER_PATH_MAX_LENGTH], char *mixer_xml_path) {
    ...
    for (int i = 0; i < kConfigLocationListSize; i++) {
        ...
        if (F_OK == access(full_config_path, 0)) {
            strcpy(mixer_xml_path, full_config_path);
            return;
        }
    }
    // if no file was found, copy file_name to mixer_xml_path
    // that way, crashes due to empty mixer_xml_path are avoided
    strcpy(mixer_xml_path, file_name);
